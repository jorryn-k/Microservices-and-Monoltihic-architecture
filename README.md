...
Separate services handle specific tasks like User recommendations, Billing, Search and others, each having their own database. In order to prevent a single point of failure, each service has its own database. This ensures that if User recommendations fails, the user can still search and watch movies.
